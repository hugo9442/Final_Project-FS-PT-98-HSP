...
const NewActionForm = ({ issueId, apartmentId, token, onSuccess, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isNaN(amount) || amount < 0) {
      ...
      setIsSubmitting(false); // <-- loading OFF si error de validación
      return;
    }
    ...
    try {
      ...
      await swal({
  title: 'Acción creada',
  text: 'La actuación se ha registrado correctamente.',
  icon: 'success',
  button: true
});
      ...
      setIsSubmitting(false); // <-- loading OFF siempre
    }
  };
    ...
    <>
      <form onSubmit={handleSubmit} className="p-3 border rounded bg-white mt-2">
        <div className="row g-3">
          <div className="col-md-6">
            <input name="action_name"
              placeholder="Nombre de actuación"
      ...
      {isSubmitting && (
        <div className="mt-3 text-center">
          ...
          <p className="mt-2">Guardando, por favor espera...</p>
        </div>
      )}
