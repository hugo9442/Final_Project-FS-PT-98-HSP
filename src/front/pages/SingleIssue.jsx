...
export const SingleIssue = props => {
  ...
  const { apartment_id, issue_id } = useParams();
  ...
  const handlegetIssuesActionsByapartmetId = async () => {
    try {
      const data = await apartments.getIssuesActionsByApertmentId(parseInt(apartment_id), store.token);
      if (!data.error) {
            ...
            <div className="p-4 border rounded bg-light">
              ...
              <div className="form mt-2">
                ...
                <ul className="list-group mt-2">
                  <h2>Incidencias</h2>
                  {
                    store && store.singleIssues.issues && store.singleIssues.issues
                    .filter(issue => issue.issue_id === parseInt(issue_id))
                    .map((item) => {
